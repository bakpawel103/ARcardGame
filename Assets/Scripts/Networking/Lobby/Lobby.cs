...
{
    PhotonNetwork.JoinOrCreateRoom($"Room{Random.Range(0, 1000)}", new RoomOptions{IsVisible = true, IsOpen = true, MaxPlayers = 2}, TypedLobby.Default);
}
...
private void GenerateRoomsList()
{
    ...
    Debug.Log("Rooms count: " + createdRooms.Count);
    
    for (int i = 0; i < createdRooms.Count; i++)
    {
        GameObject itemPrefabInstance = Instantiate(roomsListButtonPrefab, new Vector3(0, 0, 0), Quaternion.identity);
        var i1 = i;
